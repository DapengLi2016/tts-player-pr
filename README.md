...
```
  Player creation config sample: [CreatePlayerConfigSample.json](TTSPlayerManagementClient/Config/CreatePlayerConfigSample.json)
   2. List all players:
```console
...
```
  Demo config sample: [CreateDemoConfigSample.json](TTSPlayerManagementClient/Config/CreateDemoConfigSample.json)
...
# Authentication
Two authentication options, one is using subscription key, the other is using auth token.
## 1. For using authentication key
Run the command with parameters:
```console
     --region [RegionIdentifier] --subscriptionKey [YourSpeechResourceKey]
```

## 2. For using auth token with default credential
1. **Enable custom domain for the speech resource**
   [Open the speech resource in Azure Portal] => Networking => [Generate Custom Domain Name] => fill the custom domain name

2. **Assign your [Cognitive Services Speech Contributor] to your account or managed identity**
   [Open the speech resource in Azure Portal] => [Access control (IAM)] => grant [Cognitive Services Speech Contributor] role to [Your account] or [Managed identity]

3. **Run tool with below command with [Your account] or [Managed identity]**
Run the command with parameters:
```console
     --region [RegionIdentifier] --customDomainName [YourCustomDomainName]
```

- If step 2 is not assigned, then it may get below error
```console
Querying players:
https://[YourCustomDomain].cognitiveservices.azure.com/texttospeech/ttsplayer/v3.0-beta1/tts-players
FlurlHttpException: {"error":{"code":"PermissionDenied","message":"The principal `xxx@xxx.com` lacks the required data action `Microsoft.CognitiveServices/accounts/TTSPlayer/TTSPlayers/read` to perform `GET /texttospeech/ttsplayer/tts-players` operation."}}
Flurl exception status code: 401
Failed to run with exception: Call failed with status code 401 (PermissionDenied): GET https://[YourCustomDomain].cognitiveservices.azure.com/texttospeech/ttsplayer/v3.0-beta1/tts-players
...
| AllowedVoiceNameList | Yes | Predefined voice name allowed list, maximum 100 supported, only the voice name in this list can be used in synthesis API requset, please go to [Voice Gallery] in [speech portal](https://speech.microsoft.com/) to get voice name. |
| AllowedVoiceStyleList | Yes | Predefined voice style allowed list, including "general" style for default, maximum 100 supported, only the style in this list can be used in synthesis API requset, please go to [Voice Gallery] in [speech portal](https://speech.microsoft.com/) to get supported styles of the voice. |
